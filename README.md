...
- 현재 자동 chain은 `run_nemotron3_base_decision.sh` → `run_nemotron3_post_decision_probe.sh` → `run_nemotron3_public_pipeline.sh` 순서로 대기한다. Nemotron gate를 통과하면 public HN dataset 배포, 300-step LoRA, public checkpoint watcher까지 이어지고, gate가 Qwen을 선택하면 Nemotron mutation을 건너뛰고 Qwen exact-resume 대상으로 남긴다.
