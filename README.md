...
| MTEB/FAISS 평가 환경 | NFS `.venv-mteb` 복원; MTEB 2.18.0, FAISS 1.14.3, NumPy 1.26.4, Transformers 5.12.1 import gate·전체 test 208/208 통과 | [`bootstrap_mteb_env.sh`](scripts/bootstrap_mteb_env.sh) |
...
| private checkpoint watcher | Qwen step-250/500/750/1000 full-payload finite 검증·private upload·원격 재검증 완료: HF commits `7da3a573`/`ea613d32`/`adf013c9`/`363fede6`, manifest/adapter SHA exact match. 후속 LoRA는 clean winner 전체 모델의 private Hub commit을 exact base로 결속하고 자동 watcher+종료 시 reconciliation 실행 | [private watcher](docs/31_PRIVATE_CHECKPOINT_WATCHER.md) |
