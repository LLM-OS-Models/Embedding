...
선택 winner의 training manifest, mode-0600 token file, final private upload 또는 campaign-result
Git push 중 하나라도 실패하면 nonzero로 종료한다. selection JSON만 존재하는 상태를 campaign
완료로 인정하지 않는다. 최종 publisher 역시 source model directory를 직접 올리지 않고 위와
같은 allowlisted isolated staging을 사용한다. completion report의 model path/weight SHA가 exact
winner와 같고 `private:true`, `remote_manifest_exact:true`, `remote_file_set_exact:true`, 40자리
Hub commit을 모두 증명해야 frontier queue가 완료 메시지를 낸다.
