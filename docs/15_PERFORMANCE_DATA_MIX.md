...
| natural-question heuristic | 487,869 (48.79%) |
| comma keyword list | 253,598 (25.36%) |
| short search/title | 159,236 (15.92%) |
| long statement/search | 99,233 (9.92%) |
| explicit negative 1개인 row | 607,824 (60.79%) |
| explicit negative 2–6개인 row | 22,054 (2.21%) |
| explicit negative 7개인 row | 370,058 (37.01%) |
| exact query 재등장(첫 발생 제외) | 1,678 (0.17%) |
| exact positive 재등장(첫 발생 제외) | 147,763 (14.78%) |
...
instruction을 제외한 query 문자 길이는 p50/p95 `30/91`, positive `101/765`,
negative `142/820`였다. max는 각각 `2,201/18,456/59,545` characters이므로 후속 trainer가
`truncation_strategy=right`를 명시하는 이유이기도 하다.

critical-hash decontaminated revision은 instruction 안의 실제 query body 4자 미만을
모두 다음 유효 source row로 교체해 **0 rows**다. 이전 revision의 905 rows와
50K/200K에서 관찰한 짧은 body 문제를 builder 수준에서 해소했다.
