...
- SQuADKorV1 고정 Sionic full-run protocol: **0.92018**.
- 공개 reference: Qwen3-Embedding-8B `0.9063`, Comsat `0.9168`.
- Sionic 9 전체: 2026-07-18 00:26 KST 완료, macro NDCG@10 **`0.732212`**.
  MIRACL `.64994`, Mr.TyDi `.49324`, MLDR `.33463`, AutoRAG `.88550`,
  Ko-StrategyQA `.79387`, PublicHealthQA `.82497`, Belebele `.95209`,
  SQuADKorV1 `.92018`, LawIRKo `.73549`다. Comsat `.7930` 대비 `-.060788`이므로
  raw Nemotron을 그대로 채택하지 않는다.
- MLDR에서 nominal batch 64는 유지하고 OOM 1회 뒤 encoder microbatch 32로 완료했다.
  최종 summary는 `embedding_oom_retries=1`, `minimum_effective_batch_size=32`를 기록한다.
- Qwen 200K 학습은 Nemotron/Qwen/Comsat clean base decision artifact가 끝날 때까지
  재개하지 않는다.
...
Nemotron-3가 base gate를 통과할 때만 사용하는 공개 LoRA 경로는
...
현재 자동 chain은 gate가 허용할 경우 public adapter checkpoint 업로드까지 담당한다.
모든 중간 checkpoint와 병합·최종 모델도 public이다. 그 marker만으로 목표를 완료
처리하지 않는다. 다음 필수 단계는 same-step heldout loss 기반 checkpoint 선택,
