...
Qwen base의 local official run은 `--registered-loader`를 반드시 사용한다. 이 경로는
MTEB registry revision `4e423935c619ae4df87b646a3ce949610c66241c`의
`InstructSentenceTransformerModel`과 task metadata/fallback instruction을 그대로
사용한다. 내부 SentenceTransformer의 `encode`에만 exact NPY cache를 부착하며 prompt,
...
Qwen 등록 loader 전체 6종은 다음과 같이 실행한다.
