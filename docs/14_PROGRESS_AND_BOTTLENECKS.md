...
> **2026-07-17 19:06 KST 중단 정정:** 새 Qwen run도 `1875/3123`에서 외부 종료됐다.
> 마지막 full resume checkpoint는 `1750`이고 약 125 step만 재계산 손실이다. 오류/OOM은
> 없었다. 잔존 frontier polling wrapper는 오기동 방지를 위해 종료했다. 현재 GPU는
> Nemotron-3 Sionic9 full 비교를 resumable chunk cache로 실행 중이며, 결과 전까지 Qwen을
> 재개하지 않는다. 최신 사실과 exact 명령은
> [Nemotron-3 결정 문서](36_NEMOTRON3_KOREAN_BASE_DECISION_2026-07-17.md)를 우선한다.
...
runtime의 SDPA가 다시 선택됐지만 production은 중단 상태다. checkpoint 선택은
...
|---|---|---|---|
| `performance/non-commercial` | 역사적 비교·방법 탐색 | 공개 재배포 blocker가 있는 기존 composite는 local only | 사용 source와 in-domain task를 공개하고 zero-shot SOTA라고 부르지 않음 |
| `clean/release` | 실제 공개 성능 모델 | provenance/license/attribution·row별 재배포 허용 및 decontamination gate 통과 row만 | dataset/checkpoint/final model 모두 public, 권리·회귀 동시 보고 |
