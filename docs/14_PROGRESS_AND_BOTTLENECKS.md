...
H100 100%, trainer-reported 약 59.30GiB(`nvidia-smi` 약 61.7GiB), 약 23.1초/step이며
160/800 step의 잔여 추정은 약 4시간 8분이다. 200K와 1M
...
step 120 `0.00359774` → step 160 `0.00361422`로 소폭 악화됐다. 160-step margin은
`0.04057217`, mean negative/positive similarity는 `0.21843199/0.74918568`이다.
현재 50K best는 step 80이며 10K best `0.00338515`보다 아직 나쁘므로 continual
promotion gate는 열리지 않았다. 160 step은 실제 train rows의 약 23%만 본 시점이어서
scale curve를 중단하지 않되, 후반 checkpoint가 이 기준을 실제로 넘는 경우에만 200K가
50K에서 이어 학습한다.
...
| LoRA r64 | 174.588M | 10K **22.17GiB**, 50K long mix trainer **59.30GiB** / device **61.7GiB** 실측 | 10K 완료, 50K 160/800 실행 중 |
