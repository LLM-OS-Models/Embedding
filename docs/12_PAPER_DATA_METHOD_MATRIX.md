...
| [Granite Embedding Multilingual R2](https://arxiv.org/abs/2605.13521) · [311M card](https://huggingface.co/ibm-granite/granite-embedding-311m-multilingual-r2) | multilingual ModernBERT, bidirectional/CLS, 311M·768-d와 vocabulary-pruned 97M·384-d, 32K, FA2; 311M은 MRL 768/512/384/256/128. Korean은 retrieval pair로 강화한 52개 언어에 포함 | 200+언어 pretraining 뒤 web title-body, permissive public pair, IBM internal pair, IBM synthetic multilingual/long pair. source별 row 수와 재배포 가능한 corpus manifest는 미공개 | card는 multi-teacher KD, contrastive fine-tuning, MRL, model merging을 명시. 97M은 311M 계열 vocabulary selection/pruning과 KD로 축소 | weights Apache-2.0. paper/card는 공개됐지만 exact training code/data는 미공개이며 Gemma3-derived tokenizer 조건과 IBM 내부 data를 분리해야 함 | 131-task board에서 311M Borda 68위라 8B 절대성능 backbone을 바꾸지 않는다. MRL/KD/merge 근거로 참고하고 `060` 효율 frontier·한국어 장문 baseline에만 추가 |
...
| [Scaling and Stabilizing Large-Scale EBR](https://arxiv.org/abs/2607.10096) | 8 GPU differentiable all-gather cross-batch negative, current-model ANN→3-class cross-encoder→product metadata/token-overlap hybrid mining, 작은 legacy dual encoder의 temperature-1 score distribution을 큰 새 backbone에 KL로 먼저 옮기는 warm-start를 결합 | 단일 H100에서는 all-gather를 그대로 쓸 수 없어 queue4096 challenger로 음성 다양성을 비교한다. ANN→Qwen3-Reranker score-quantile KD는 직접 채택한다. product-type/brand/size heuristic은 한국어 범용 data에 복사하지 않고 source/task/document provenance를 request schema에 실제 결속한 뒤에만 metadata-hybrid라고 부른다. 현재 clean-selected 8B에서 1M을 잇는 것은 별도 legacy-KL warm-start가 아님 |
...
| Granite Multilingual R2 | Apache-2.0 weights | exact training code 미공개 | public/internal/generated 범주만 공개, row manifest 미공개 | model baseline 가능; IBM data를 재현·재배포했다고 주장하지 않음 |
...
| Scaling and Stabilizing Large-Scale EBR | Walmart 내부 DistilBERT/GTE-base 계열 | paper algorithm 공개, production code 미공개 | 익명 engagement 4M query·약 900M pair 및 product index 비공개 | cross-batch/hybrid/KL의 개념과 ablation만 참고; 내부 data·heuristic 재현 claim 금지 |
...
| [Scaling and Stabilizing Large-Scale EBR](https://arxiv.org/abs/2607.10096) | local batch 밖의 negative, current-model iterative mining, cross-encoder와 구조 metadata를 함께 쓰는 false-negative guardrail, legacy score KL warm-start가 각각 추가 이득 | Walmart 비공개 engagement query 4M에서 top impressed product 300개, 약 900M pair. query당 positive 7/offline negative 3/cross-batch negative 5, local batch 72×8 A100, LR `2e-6`. progressive offline ablation에서 DR→CB→HHN→큰 backbone→warm-start 순으로 EM Recall@20 `+2.67→+2.88→+3.15→+3.74→+4.00%` | `030`: 단일 GPU cross-step queue4096, current-student ANN, pinned Qwen reranker, score-quantile KL로 검증. e-commerce PT/brand/size rule은 범용 한국어에 직접 이전하지 않는다. 별도 backbone 교체가 생길 때만 clean legacy dual-encoder score KL을 새 ablation으로 추가 |
...
- [Granite Embedding Multilingual R2](https://arxiv.org/abs/2605.13521), [311M official model card](https://huggingface.co/ibm-granite/granite-embedding-311m-multilingual-r2)
