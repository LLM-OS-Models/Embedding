...
| Python 환경 | NFS `.venv-hf-tools`, `.venv-train-fa2`, `.venv-mteb` 복원; 8B backward와 전체 test 184/184 통과 |
...
않고 safe-merged full weight를 FP32로 누적하는 방식으로 구현했다. general/combined 50:50,
general 50%+specialist 5개 각 10%, general/combined 각 25%+specialist 각 10%, combined
50%+specialist 각 10%의 네 coefficient는 평가 전에 고정한다. architecture/ST contract,
...
그 직전에 `run_model_soup_queue.sh`가 fixed full-weight soup 네 종을 만들고 같은 selector의
...
`token=False`를 명시하며, upload process만 메모리에서 읽는다.
