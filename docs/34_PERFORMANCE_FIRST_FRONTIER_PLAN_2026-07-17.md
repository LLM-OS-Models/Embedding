...
| fixed multidomain selector | `LLM-OS-Models2/korean-embedding-multidomain-selection-heldout-v1@d261e1e3ff64e13828e73017fe2c312aae575709` | finance 900 + knowledge 1,000; private selection-only |
...
Comsat 후보는 Grade-I legal holdout, 고정 finance/knowledge와 noise robustness에서 비교하며, selection 파일이
...
평균을 같은 Grade-I legal/multidomain/robustness gate에서 비교한다. 모델 이름에 맞는 실제 training
...
각 run의 checkpoint를 legal/multidomain/robustness selector로만 비교하고 public Sionic 9를 selector로 쓰지 않는다.
...
- 1M 원본과 세 KD 후보는 Grade-I legal/multidomain/robustness에서만 비교한다. 이 단계에서는 Sionic 9와
  공식 Korean 6을 호출하지 않으며, local selector winner만 target/legal queue의 general base가 된다.
- exact KD train/audit/request/score artifacts는 SHA/row/admissibility를 다시 확인하고
  ...
  upload한다.
- selector의 exact winner는 model shard SHA, merge parity, legal/multidomain/robustness summary+ranks,
...
adapter 파일의 hash를 다시 검증한다. single best와 평균 모델은 둘 다 Grade-I legal/multidomain/robustness
...
archive의 무결성 검증된 모든 checkpoint를 동일 legal 10K, multidomain 1.9K와 paired noise 6조건으로
...
아니다. 최종 local winner에만 Sionic 9/공식 Korean/comprehensive evaluation과 private
...
| local retrieval | legal best `-0.005` guard 안에서 finance/knowledge macro best `-0.002`, robustness best `-0.002`, intrusion min `+0.001` |
...
local selector winner를 base로 쓰면 그 모델의 단 하나뿐인 passing evidence에서 이 목록을 재귀
...
6. 200K Qwen과 Comsat 두 run을 처음부터 시작한다. — **Qwen active; step 1750까지 private
   immutable commit·LFS/manifest/lineage 독립 재검증 완료, Comsat 및
   post-eval→1M→target/legal 직렬 queue armed**
7. winner에 1M, current-student wide mining, reranker stratified KD를 순서대로 적용한다. — **코드·자동 queue·legal-guard multidomain selector 완료, 실제 실행 대기**
