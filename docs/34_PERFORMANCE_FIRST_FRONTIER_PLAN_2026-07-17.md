...
| Python 환경 | NFS `.venv-hf-tools`, `.venv-train-fa2`, `.venv-mteb` 복원; 8B backward와 전체 test 201/201 통과 |
...
그 직전에 `run_model_soup_queue.sh`가 fixed full-weight soup 최대 일곱 종을 만들고 같은 selector의
