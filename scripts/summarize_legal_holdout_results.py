...
def load_rows(root: Path) -> list[dict[str, Any]]:
    ...
    return sorted(latest.values(), key=lambda row: (preferred.get(row["model"], 2), row["model"]))
...
def markdown(rows: list[dict[str, Any]]) -> str:
    ...
    lines = [
        "| Model | NDCG@10 | Recall@10 | MRR@10 | Recall@100 | Mean rank |",
        "|---|---:|---:|---:|---:|---:|",
    ]
    for row in rows:
        metrics = row["metrics"]
...
def main() -> None:
    ...
    if args.readme:
        update_readme(args.readme, markdown(rows))
