...
def validate(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if getattr(args, "mining_manifest", None):
...
def dataset_card(args: argparse.Namespace, validated: dict[str, Any]) -> str:
    ...
    if args.mining_manifest:
        construction = (
            "current-student FAISS 후보의 exact float32 score를 재계산하고, "
            "positive-relative filter와 score-rank quantile로 hard negative를 선택했다."
        )
    else:
        construction = (
            "source-native query/positive 관계와 deterministic bootstrap negative로 "
            "컴파일했다. 최종 학습 전 current-student hard-negative mining을 수행해야 한다."
        )
    release_eligible = manifest.get("release_eligible") is True
    public = bool(getattr(args, "public", False))
    use = "public redistribution and model training" if public else "private research"
    batch_line = (
        f"- batch size: `{manifest['batch_size']}`\n"
        if manifest.get("batch_size") is not None
        else ""
    )
    adaptation_line = (
        f"- benchmark adaptation: `{adaptation}`\n" if adaptation else ""
    )
    optional_files = []
    if getattr(args, "mining_manifest", None):
    ...
    if getattr(args, "mining_audit", None):
        optional_files.append(
            "- `metadata/mining_audit.jsonl`: per-input threshold와 selected document hashes"
        )
    if getattr(args, "quality_audit", None):
        optional_files.append(
            "- `metadata/training_data_quality_audit.json`: final train/provenance 품질 계약"
        )
    if getattr(args, "benchmark_overlap_audit", None):
        optional_files.append(
            "- `metadata/benchmark_overlap_audit.json`: 원문 없는 exact text-hash overlap 감사"
        )
    optional_file_text = "\n".join(optional_files)
    return f"""---
language:
...
- rows: **{validated['rows']:,}**
{batch_line}{adaptation_line}- release eligible: **{str(release_eligible).lower()}**
