...
2026-07-12 critical-hash revision부터 Sionic 9 + 공식 Korean 6의 text-only blocklist를
builder에 적용한다. Retrieval evaluation query text는 모든 source에서 차단하고,
non-retrieval task는 source가 같은 `trained_on_tasks`를 명시한 공식 train-family text만
expected exposure로 허용한다. 이전 1M에서 critical row 2,839개를 제거하고 같은
source의 다른 row로 cap을 다시 채워 규모와 source mixture를 유지했다.

- rejected rows: 2,839
- matched text occurrences: retrieval eval query 26,
  undeclared KLUE-STS-family text 8,435
- final ordered critical query/evaluation-text match: **0**
- declared train-family match: 고유 29,226 hash
- retrieval corpus-only match: 고유 23,379 hash

따라서 final artifact는 evaluation query/text critical overlap은 없지만 task train-family와
shared retrieval corpus에 노출된 target-adapted performance dataset이다.
...
- `data/train.jsonl` SHA-256:
  `056abaf6b72d7521f9f60483e7ee1267011e3fe4803ee0429e19db4df323d2fa`
- `metadata/provenance.jsonl` SHA-256:
  `81dac008cc9523cb64983aaa8f623b7cb81c5075f6cc045ac6fd74e04e6bd4f4`
- exact trainer order: 999,936 rows
- ordered train SHA:
  `7f2641a0a0256e89b2abb3462aa2c8b415b3e605fd4ca413178d2ad4abfc9009`
- ordered provenance SHA:
  `b036f6ef28d1a09d16aab4cedbb896d44e4bccd98a0dee94959aa0b4bae56646`
