...
clean-first selection 뒤 [`publish_private_clean_candidate.py`](../../scripts/publish_private_clean_candidate.py)가
exact winner만 `LLM-OS-Models2/qwen3-embedding-8b-ko-reranker-kd-clean-winner-v1-private`에
비공개로 백업한다. model shard/merge report, Grade-I clean·robustness summary와 10K rank
evidence, selection policy, 실제 training manifest SHA가 모두 맞아야 한다. tokenizer 파일은
허용하지만 optimizer/scheduler/trainer state/credential/symlink는 거부한다. upload worker는
1개이며 다음 GPU target 학습과 겹쳐도 CPU/NFS를 과점하지 않도록 background로 실행한다.
